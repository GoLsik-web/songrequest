...
//
// Теперь двадцать пять секунд, то есть примерно два запроса в минуту, как
// попросил владелец. Пауз между треками от этого не появилось: приложение
// знает длительность заказа и просыпается ровно к его концу (см. awaitNap в
// internal/player). Частый опрос нужен был только для того, чтобы заметить,
// что стример переключил музыку руками в самом Spotify, — и это приложение
// теперь замечает с опозданием до полуминуты. Такова цена, и она заметно
// меньше суток без Spotify.
const (
	orderPoll = 25 * time.Second
...
// стрима таких запросов набегают тысячи. Потом было пятнадцать секунд, а
// 10.09 ограничение прилетело снова, уже на двадцать один час, — и стало
// ясно, что экономить надо всерьёз. Тридцать секунд при открытой панели: это
// два запроса в минуту, а перемотку своей музыки стример видит в самом
// Spotify, который у него на том же экране.
const (
	ownRefreshPanel  = 30 * time.Second
