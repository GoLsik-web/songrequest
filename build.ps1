...
if (-not $NoZip) {
    ...
    foreach ($doc in @("docs\Инструкция-Spotify.html", "docs\Чек-лист-проверки.txt",
                       ...
                       "docs\Инструкция-Обход-блокировок.html")) {
...
Отдельно: «Инструкция-Обход-блокировок» — открывай её ТОЛЬКО если приложение
пишет, что не может достучаться до Spotify (так бывает у российских
провайдеров). Там всё сводится к одному: вставить в приложение ключ от своего
VPN. Отдельные программы обхода и настройки Windows больше не нужны.
...
Откроется окно программы. Крестик его не закрывает, а прячет к часам,
справа внизу: заказы продолжают работать. Вернуть окно - щелчок по значку
возле часов, закрыть совсем - правой кнопкой по значку и "Выход".
